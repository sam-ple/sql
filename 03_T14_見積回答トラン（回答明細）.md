...
SELECT
    ...
    , T14.is_declined AS '辞退区分' -- 0:未回答または回答済 1：見積辞退
    , T14.reply_quantity AS '回答数量' -- 
    , T14.unit_price AS '単価' -- 
    , T14.amount AS '金額' -- 
    , T14.due_date AS '納期' -- 
    , T14.parts_list_pattern AS '部品リスト　パターン' -- 1:概算　2:受注
    , T14.adoption_type AS '採用区分' -- 0:未決定 1:採用　2:不採用
    , T14.adoption_num AS '採用数' -- 
    , T14.reason AS '採用・不採用理由' -- 採用・不採用の理由
    , T14.send_date AS 'メール送信日' -- 
    , T14.sender_pic_code AS '送信担当者コード' -- 
    , T14.rfq_no_at_inquiry AS '引合時　見積依頼番号' -- 受注時、見積回答を引き当てた際の退避用
